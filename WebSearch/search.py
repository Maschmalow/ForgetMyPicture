...
__version__ = "0.2"
URL = 'https://www.google.fr/search?&safe=off&tbm=isch&q=bonjour+%22au+revoir%22'
...
class pygoogle:
    ...
    def __search__(self,print_results=False):
        ...
        self.logger.debug('search: "%s"'%(self.query))
        q = urllib.urlencode(args)
        search_results = urllib.urlopen(URL+q)
        for line in search_results:
            results.append(line)
            
        if search_result.getcode() != 200:
            self.logger.debug('responseStatus is not 200')
            self.logger.error('responseStatus : %s'%(search_result.getcode()))
            return results
        if print_results:
